...
- 协议：[`config/protocol_v1.json`](config/protocol_v1.json)（当前为 `G1_EXTERNAL_FDS_RUN_REQUIRED`，正式批算仍未授权）
...
G1 本地软件合同已完成并生成四个待外算输入；当前仍无目标版本 FDS 实跑证据。只有这批短试算通过质量门后才能继续 G2，现有设计草案不得用于正式批算。
