...
> 当前正式证据基线：**FDS 6.9.1**。2026-08-01 用户明确决定对 firemodels
> 官方外部验证快照和已经回传的主体/先导结果统一采用其现有 6.9.1 版本，
> 不再仅因未使用 6.10.1 降级或要求重跑。
> FDS 6.10.1（Revision `FDS-6.10.1-0-g12efa16-release`）的 2026-07-29
> 本机短试算结论继续作为该版本的兼容性记录，不得改写成 6.9.1 实跑结论。
> 新的正式工况默认按 6.9.1 审计；修改生成器后仍须以实际目标版本短试算。
...
| 路径 | 角色 |
|---|---|
| `阶段一/src/tunnel_config.py` | 几何、物理量及 FDS 量名常量的单一事实源 |
| `阶段一/src/generate_fds_case.py` | `.fds` 生成器；所有工况由它生成 |
| `阶段一/02_FDS基准模型/tunnel_benchmark.fds` | 生成器产出的基准输入文件，不作为独立源手改 |
| `阶段一/02_FDS基准模型/MODEL_SETTINGS.md` | 模型设置和首次运行检查说明 |
| `阶段一/src/fds_io.py` | FDS CSV 读取和单位归一化 |
| `阶段一/src/check_convection_ratio.py` | 从 `_hrr.csv` 计算辐射与火源对流比例 |
| `阶段一/03｜04｜05_*/*.csv` | 网格敏感性、长度边界和外部试验工况参数 |
...
## 1. FDS 输入文件规范（6.9.1 正式证据基线；6.10.1 兼容性实测）
...
### 1.13 完整工况运行说明必须标明输入与输出路径

凡提供正式、完整或批量 FDS 工况的运行方案、命令或服务器提交说明，必须在每组命令旁同时列明：

1. **输入文件位置**：待运行 `.fds` 的完整本机路径；用于服务器计算时，另给出建议的服务器输入路径或上传后的相对路径。
2. **输出目录位置**：FDS 实际工作目录的完整路径。运行命令必须先进入该目录，或使用能够明确指定工作目录的参数，避免输出散落或出现重复 CHID 嵌套目录。
3. **关键输出文件位置**：至少列出 `<CHID>.out`、`<CHID>_devc.csv`、`<CHID>_hrr.csv`、`<CHID>.smv`，并按需说明切片 `<CHID>_*.sf*`、边界 `<CHID>_*.bf` 和结束标记 `<CHID>.end`。
4. **服务器搬运说明**：明确哪些输入文件需要上传、计算完成后哪些输出应下载用于本项目后处理。默认至少上传 `.fds`，至少下载 `.out`、`_devc.csv` 和 `_hrr.csv`；需要 Smokeview 或场分析时再下载 `.smv`、切片和边界文件。
5. **CHID 对应关系**：批量工况应给出 CSV、生成后的 `.fds`、输出目录和 CHID 命名关系，不能只给一条抽象运行命令。

FDS 默认把输出写入启动时的工作目录。Windows 命令应使用完整路径并显式 `cd /d` 到输入文件所在目录；Linux/HPC 命令应显式 `cd` 到工况目录，或在作业脚本中设置工作目录。不得使用会无意创建多层同名 CHID 目录的参数。
...
- 网格敏感性 9 个正式工况统一使用 `n_mesh_x=11`、`n_mesh_y=1`、`n_mesh_z=2`，即 `11×1×2=22 MESH`；参数由 `阶段一/03_网格敏感性/grid_sensitivity_cases.csv` 提供，生成逻辑由 `generate_fds_case.py` 实现。
...
- [ ] FDS 日志显示的版本与本批证据基线一致；当前正式基准为 FDS 6.9.1。
