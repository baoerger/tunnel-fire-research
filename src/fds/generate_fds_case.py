"""
generate_fds_case.py — 隧道火灾 FDS 输入文件生成器
...
_DESIGN_ONLY_STATUS = "DESIGN_ONLY_NOT_AUTHORIZED_FOR_FORMAL_RUN"
...
def normalize_case(spec, row_number=None):
    ...
    case_kind = str(raw.get("case_kind") or "fire").strip().lower()
    if case_kind not in ("fire", "background"):
        raise ValueError(f"{prefix}: case_kind={case_kind!r} 仅支持 fire/background")
    for field in ("dx", "L", "W", "H", "T_end"):
        if values[field] <= 0:
            raise ValueError(f"{prefix}: {field} 必须大于 0，当前为 {values[field]}")
    if case_kind == "fire":
        for field in ("Q", "Df"):
            if values[field] <= 0:
                raise ValueError(f"{prefix}: 火灾工况 {field} 必须大于 0")
    elif abs(values["Q"]) > 1e-12 or abs(values["Df"]) > 1e-12:
        raise ValueError(f"{prefix}: background 工况要求 Q=0 且 Df=0")
    if values["U"] < 0:
    ...
    mesh_profile = str(raw.get("mesh_profile") or "legacy_uniform").strip().lower()
    if mesh_profile not in ("legacy_uniform", "no_wind_symmetric_v1"):
        raise ValueError(f"{prefix}: 未知 mesh_profile={mesh_profile!r}")
    if mesh_profile == "no_wind_symmetric_v1":
        if not all(math.isclose(values[key], expected, abs_tol=1e-8)
                   for key, expected in (("L", cfg.L), ("W", cfg.W), ("H", cfg.H))):
            raise ValueError(f"{prefix}: no_wind_symmetric_v1 仅适用于 100×10×5 m")
        if abs(values["U"]) > 1e-8:
            raise ValueError(f"{prefix}: no_wind_symmetric_v1 只允许 U=0")

    if mesh_profile == "legacy_uniform":
        axis_specs = (
            ("x", values["L"], values["n_mesh_x"], bounds[0] if bounds else None,
             bounds[1] if bounds else None),
            ("y", values["W"], values["n_mesh_y"], bounds[2] if bounds else None,
        ...
        _symmetric_mesh_parts(values["dx"])
        if bounds:
            x_internal = (9, 18, 27, 36, 64, 73, 82, 91)
            crossing = [edge for edge in x_internal if bounds[0] < edge < bounds[1]]
            if crossing:
                raise ValueError(f"{prefix}: 对称 MESH x 边界 {crossing} 切穿燃烧器")
    ...
    if output_profile not in ("light", "heavy"):
        raise ValueError(f"{prefix}: output_profile={output_profile!r} 仅支持 light/heavy")
    values["output_profile"] = output_profile
    if bounds:
        area = (bounds[1] - bounds[0]) * (bounds[3] - bounds[2])
        values.update({
            "Af_discrete_m2": area,
            "Df_discrete_m": math.sqrt(4.0 * area / math.pi),
            "xf_actual_m": (bounds[0] + bounds[1]) / 2.0,
            "yf_actual_m": (bounds[2] + bounds[3]) / 2.0,
        })
    else:
        values.update({"Af_discrete_m2": 0.0, "Df_discrete_m": 0.0,
                       "xf_actual_m": values["x_fire"], "yf_actual_m": values["W"] / 2.0})
    return values
...
def _symmetric_mesh_parts(dx):
    ...
    for x0, x1 in zip(x_bounds, x_bounds[1:]):
        ...
        for z0, z1 in zip(z_bounds, z_bounds[1:]):
            extents = (x1 - x0, cfg.W, z1 - z0)
            cells = []
            for extent in extents:
                raw = extent / dx
                if not math.isclose(raw, round(raw), rel_tol=0.0, abs_tol=1e-8):
                    raise ValueError(
                        f"no_wind_symmetric_v1 边界跨度 {extent:g} 不能被 dx={dx:g} 整除"
                    )
...
def _fmt_mesh(L, W, H, dx, n_mesh_x=1, n_mesh_y=1, n_mesh_z=1,
              mesh_profile="legacy_uniform"):
    if mesh_profile == "no_wind_symmetric_v1":
        lines = []
        for index, (xb, ijk) in enumerate(_symmetric_mesh_parts(dx), start=1):
            lines.append(
                f"&MESH ID='MESH_{index:02d}', IJK={ijk[0]} {ijk[1]} {ijk[2]}, "
...
# 3. 几何：壁面、洞口、燃烧器
# ----------------------------------------------------------------------------
def _fmt_geometry(L, W, H, bounds=None):
    """
    ...
    if bounds is None:
        lines.append(
            f"&VENT XB={cfg.X0:.2f} {L2:.2f} {cfg.Y0:.2f} {W2:.2f} "
            f"{cfg.Z0:.2f} {cfg.Z0:.2f}, SURF_ID='WALL' /"
        )
    else:
        x0, x1, y0, y1 = bounds
...
def _fmt_devices(x_fire, sensor_xs, L=None, W=None, H=None, hrr_region=None,
                 case_kind="fire", sensor_profile="standard"):
    lines = []
    ...
    # 4a 顶棚中心线气体温度测点（§1.7）
    if sensor_profile == "no_wind_global_v1":
        for ratio, height in zip(cfg.NO_WIND_TEMPERATURE_HEIGHT_RATIOS,
                                 cfg.no_wind_temperature_heights(H)):
            prefix = f"T{int(round(100 * ratio)):02d}"
            for x in sensor_xs:
                lines.append(
    ...
    if case_kind == "fire" and hrr_region is None:
        L2 = cfg.X0 + L
        W2 = cfg.Y0 + W
        H2 = cfg.Z0 + H
...
# ----------------------------------------------------------------------------
def _fmt_slices(L, W, H, x_fire, sensor_profile="standard", output_profile="light"):
    lines = []
    y_mid = W / 2.0
    lo, hi = cfg.measurement_region(L, H)
...
def render_fds(chid, Q, U, Df, dx, L=None, W=None, H=None, x_fire=None, T_end=None,
               ...
               n_mesh_z=1, rnd_seed=0, sensor_profile="standard",
               case_kind="fire", mesh_profile="legacy_uniform",
               output_profile="light", _normalized=None):
    """
    渲染一份完整 FDS 输入字符串。
    ...
    normalized = _normalized or normalize_case(dict(
        ...
        case_kind=case_kind, mesh_profile=mesh_profile, output_profile=output_profile,
    ))
    Q = normalized["Q"]
    U = normalized["U"]
    Df = normalized["Df"]
    ...
    case_kind = normalized["case_kind"]
    ...
    if case_kind == "fire":
        ...
        parts.append(_fmt_surf_burner(Q, bounds))
    ...
    parts.extend(_fmt_devices(x_fire, sensor_xs, L=L, W=W, H=H,
                              case_kind=case_kind, sensor_profile=sensor_profile))
    if output_profile == "heavy":
        parts.append(f"&BNDF QUANTITY='{cfg.QUANTITY_BNDF}' /")
...
def _run_csv(csv_path, outdir, required_only=False):
    with open(csv_path, newline="", encoding="utf-8-sig") as f:
        ...
        rows = list(reader)
        blocked = [
            (index, (row.get("chid") or "").strip())
            for index, row in enumerate(rows, start=2)
            if str(row.get("status") or "").strip().upper() == _DESIGN_ONLY_STATUS
        ]
        if blocked:
            details = ", ".join(f"第{line}行 {chid or '<missing chid>'}" for line, chid in blocked)
            raise ValueError(
                f"CSV 含 {_DESIGN_ONLY_STATUS} 草案，禁止批量生成正式输入: {details}"
            )
        if required_only:
    ...
    for mode, case, line_number in prepared_rows:
        ...
        text = render_fds(
            ...
            sensor_profile=case["sensor_profile"],
            case_kind=case["case_kind"], mesh_profile=case["mesh_profile"],
            output_profile=case["output_profile"],
            _normalized=case,
        )
        os.makedirs(outdir, exist_ok=True)
...
def main():
    ...
    ap.add_argument("--mesh-profile", choices=("legacy_uniform", "no_wind_symmetric_v1"),
                    default="legacy_uniform")
    ap.add_argument("--output-profile", choices=("light", "heavy"), default="light")
    ...
    spec = dict(chid=args.chid, Q=args.Q, U=args.U, Df=args.Df, dx=args.dx,
                ...
                case_kind=args.case_kind, mesh_profile=args.mesh_profile,
