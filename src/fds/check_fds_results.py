...
from . import fds_io
from .project_paths import FDS_RUNS_DIR, PROJECT_ROOT, output_path


DEFAULT_EXTERNAL_MAPPING = (
    PROJECT_ROOT / "data" / "reference" / "external_measurement_mapping.csv"
)
...
def _parse_input(path):
    ...
    case_kind = "fire" if re.search(r"SURF_ID\s*=\s*'BURNER'", text, re.I) else "background"
    return {
...
def check_case(case_dir, expected_version=DEFAULT_EXPECTED_VERSION, closure_tolerance=0.20,
    ...
    case_kind = meta.get("case_kind", "unknown")
    if case_kind == "fire" and (
            not required["hrr"].is_file() or required["hrr"].stat().st_size == 0):
        hard_issues.append(f"缺少或空文件: {required['hrr'].name}")
    ...
    if case_kind == "background" and not hrr_times:
        hrr_time_ok, hrr_final = True, None
    else:
    ...
    if required_temperature_channels:
        ...
        temp_keys = [key for key in devc_series
                     if re.fullmatch(r"T(?:(?:85|90|95))?_\d+", key)]
        velocity_keys = [key for key in devc_series
                         if re.fullmatch(r"U(?:95)?_\d+", key)]
        hrr_tot_key = _exact_key(devc_series, "HRR_tot")
        heat_flux_keys = [key for key in devc_series if re.fullmatch(r"Qw_\d+", key)]
        if case_kind == "background":
            required_devc_ok = bool(temp_keys and velocity_keys)
        else:
            required_devc_ok = bool(temp_keys and velocity_keys and hrr_tot_key and heat_flux_keys)
        if not required_devc_ok:
            expected = "温度和纵向速度列" if case_kind == "background" else "T_*、U_*、HRR_tot 或 Qw_* 列"
            hard_issues.append(f"DEVC 缺少{expected}")
        ...
        units_ok = (
            ...
            and (
                ...
                or (
                    ...
                    and (case_kind == "background" or (
                        _unit_ok(devc_units.get(hrr_tot_key), {"KW", "W", "MW"})
    ...
    required_hrr_ok = (case_kind == "background" and not hrr_series) or bool(hrr_key and q_radi_key)
    if case_kind != "background" and not required_hrr_ok:
        hard_issues.append("_hrr.csv 必须包含精确列 HRR 和 Q_RADI")
    elif case_kind != "background" and not (_unit_ok(hrr_units.get(hrr_key), {"KW", "W", "MW"})
    ...
    if case_kind == "background":
        hrr_candidates = []
        if hrr_tot_key and normalized_devc.get(hrr_tot_key):
            hrr_candidates.extend(normalized_devc[hrr_tot_key])
        if hrr_key and normalized_hrr.get(hrr_key):
            hrr_candidates.extend(normalized_hrr[hrr_key])
        hrr_nonzero = bool(hrr_candidates and max(abs(value) for value in hrr_candidates) > 1.0)
        if hrr_nonzero:
            hard_issues.append("背景工况检出非零 HRR")
    elif required_temperature_channels:
        hrr_nonzero = bool(
    ...
    if case_kind != "background" and not hrr_nonzero:
    ...
    if case_kind == "background":
        pass
    elif required_hrr_ok and hrr_times and target_hrr and meta["tau_q"] is not None:
