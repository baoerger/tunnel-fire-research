...
> **历史冻结（2026-08-14）：**物理解码网络已经退出当前研究范围，本目录停止扩展。
> 任何合成权重或软件指标均不得作为无风公式研究证据。
...
- `src/set_encoder.py`：单传感器 `{dT, x, m}` 与已知 U、Df、H、W、T0 输入；共享编码器；掩码 mean/max/有效数量聚合；`Q_ref exp(z_Q)` 和有界 sigmoid 位置输出；状态保存/加载。L、dx、协议和删失元数据用于证据分级，缺失或越界不阻断探索性推理。
- `src/physics_training.py`：合成参数、相关温度噪声、位置误差、随机缺测和模型残差生成；与阶段五合成前向一致的可微解码器；参数监督、温度重建和删失联合损失；工况级物理域删失与传感器 `m` 缺测掩码分离。
...
正式结论所用样本应显式携带 `L/W/H/dx`、`protocol_version`、
`domain_censor_state` 与 `parameter_target_mask`；信息不全的样本仍可用于方法开发，
但状态为 `APPLICABILITY_UNDETERMINED`，不与核心域正式指标合并。
上游删失掩掉
`kappa_u`，下游域删失掩掉 `kappa_d`；它们不能写入单个传感器，也不能用
`m=0` 伪装成随机缺测。网络性能结论同样只适用于 100 m 条件域。
