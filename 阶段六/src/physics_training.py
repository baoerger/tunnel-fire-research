...
for source_dir in (
    STAGE6_ROOT / "src", PROJECT_ROOT / "阶段一" / "src",
    PROJECT_ROOT / "阶段五" / "src",
):
...
def load_sensor_catalog(path=None):
    path = Path(path or (
        PROJECT_ROOT / "阶段一" / "07_温度测点与输出规范" / "sensor_layout.csv"
    ))
...
def main(argv=None):
    ...
    parser.add_argument("--output-dir", type=Path, default=STAGE6_ROOT / "02_合成数据与物理解码")
