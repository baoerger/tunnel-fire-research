...
- `src/direct_inversion.py`：从 Q、x_f、U、Df 和测点位置计算温升；以 `log Q`、`x_f/H` 执行粗网格、多初值局部细化、边界约束和删失损失；核心域、域外探索和元数据不完整结果分级标记，只有最终正式条件域筛选才启用严格模式。
- `src/fisher_layout.py`：中心差分灵敏度、指数空间相关噪声、Cholesky 求解、Fisher 指标、4/8/12/16 点代表布局、多场景平均/最差候选优化以及 Q—位置目标函数和剖面 SVG；共享适用域评估，但允许带标志的域外探索。
- `src/confidence_intervals.py`：局部 Hessian、卡方目标剖面、相关观测扰动、相关残差直接反演和参数 Bootstrap；适用域信息随高层结果输出，不因验证尚未完成而阻断计算。
...
域删失和传感器低温升删失含义不同：`downstream_domain_censored` 允许在
15--85 m 已观测剖面上反演 Q/x_f，但结果必须附带
`K_D_NOT_IDENTIFIABLE_FROM_THIS_DOMAIN`，不能据此报告下游衰减参数。
