...
>
> 当前模型身份：`M1_S1/A0` 是不可变人工候选基准；父机器协议与静态预检已完成，但执行补充协议待批准且两张冻结开发输入缺失；SR5 另缺同步时间块输入。最终正向公式冻结 `A_active` 尚未生成。锁箱保持 `SEALED_UNREAD_UNRUN`，G6 暂停，快速反演保持 `G7_SKIPPED_NOT_REQUESTED`。
